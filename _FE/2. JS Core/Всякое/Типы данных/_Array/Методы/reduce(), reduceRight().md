Метод `reduce()` позволяет преобразовать целевой массив. Принимает в аргументы коллбэк и стартовое значение аккумулятора коллбэка `initialValue`. Возвращает единственное выходное значение.
